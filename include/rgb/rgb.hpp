    ...
    size_t num_queries, size_t n, uint64_t depth,uint64_t max_depth)
{
...

    {
        ...
        for (int cur_iter = 1; cur_iter <= constants::MAX_ITER; cur_iter++) {
            ...
            if constexpr (isParallel) {
                ...
                gains = compute_move_gains<false>(partition, num_queries, deg1, deg2,
                    before, left2right, right2left, query_changed);
            }
    ...
    if (depth + 1 <= max_depth) {
        ...
        auto by_id = [](auto &&lhs, auto &&rhs) { return lhs.initial_id < rhs.initial_id; };
        std::sort(std::execution::unseq, G, G+n, by_id);
        progress.done(n);
    }
}
